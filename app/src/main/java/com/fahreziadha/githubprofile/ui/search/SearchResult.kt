...
import com.fahreziadha.githubprofile.ui.components.LoadMoreButton
            ...
            CustomSurface(
                ...
                LazyColumn() {
                    itemsIndexed(item) { index, searchItem ->
                        SearchItem(
                            onClick = {
                                navController.navigate(Screen.DetailScreen.route + "/${searchItem.login}")
                                viewModel.saveCacheSearch(
                                    CacheUser(
                                        id = searchItem.id,
